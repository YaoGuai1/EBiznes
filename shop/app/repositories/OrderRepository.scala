...
class OrderRepository @Inject()(dbConfigProvider: DatabaseConfigProvider, val userRepository: UserRepository, val paymentRepository: PaymentRepository)(implicit ec: ExecutionContext) {
  ...
  class OrderTable(tag: Tag) extends Table[Order](tag, "UserOrder") {
    ...
    def userIdFK = foreignKey("userId_FK", userId, user)(_.id, onUpdate=ForeignKeyAction.Cascade, onDelete=ForeignKeyAction.SetNull)
    def paymentIdFK = foreignKey("paymentId_FK", paymentId, payment)(_.id, onUpdate=ForeignKeyAction.Cascade, onDelete=ForeignKeyAction.SetNull)
